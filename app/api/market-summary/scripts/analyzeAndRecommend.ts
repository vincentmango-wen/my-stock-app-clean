...
// RSIを計算する関数（通常14日間）
function calculateRSI(closingPrices: number[], period = 14): number | null {
...
function isMACDGoldenCross(closingPrices: number[]): boolean {
...
const priceData = JSON.parse(fs.readFileSync('./price-data.json', 'utf-8')) as Record<string, number[]>;
...
// 銘柄を分析して推奨度を計算する関数
function analyzeStock(code: string, prices: number[]): StockRecommendation | null {
  if (prices.length < 26) return null; // 最低26日分のデータが必要
